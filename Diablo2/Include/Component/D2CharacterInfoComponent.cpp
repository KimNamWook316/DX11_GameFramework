...
{
	mSpawnRoomIdx = com.mSpawnRoomIdx;
...
void CD2CharacterInfoComponent::Start()
{
	...
	mEventCallBackList->resize((int)eD2CharInfoEventType::Max);
}
...
void CD2CharacterInfoComponent::Update(float deltaTime)
{
	if (mCCTime >= mCharInfo.MaxCCTime)
	{
		...
		mCCTick = 0.f;
		meCC = eD2ElementType::None;
		callEventCallBack(eD2CharInfoEventType::CCEnd);
	}
...
void CD2CharacterInfoComponent::SetExp(const int exp)
{
	mCharInfo.Exp += exp;

	callEventCallBack(eD2CharInfoEventType::ExpUp);

	if (mCharInfo.Exp >= mCharInfo.MaxExp)
	{
		mCharInfo.Exp = mCharInfo.MaxExp;

		if (mCharInfo.Level < mCharInfo.MaxLevel)
		{
			mCharInfo.Exp = 0;
			callEventCallBack(eD2CharInfoEventType::ExpUp);

			++mCharInfo.Level;
			callEventCallBack(eD2CharInfoEventType::LevelUp);

			// 레벨업하면 Hp, Mp 회복
			SetHp(mCharInfo.MaxHp);
			SetMp(mCharInfo.MaxMp);
		}
	}
}
...
void CD2CharacterInfoComponent::SetCC(eD2ElementType type)
{
	...
	callEventCallBack(eD2CharInfoEventType::CCStart);
}
